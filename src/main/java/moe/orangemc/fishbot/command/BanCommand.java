...
public class BanCommand extends SimpleCommand {
    public BanCommand() {
        super(FishBotPlugin.INSTANCE, "ban", new String[]{}, "封禁帐号", FishBotPlugin.INSTANCE.getPermissionMap().getOrCreate("command.ban"), CommandArgumentContext.EMPTY);
    }
    ...
    public void onCommand(CommandSender sender, long qq) {
        try {
            ...
            sender.sendMessage("已封禁" + player.getName() + SuffixGenerator.getSuffix());
        } catch (Exception e) {
            e.printStackTrace();
            sender.sendMessage("发生了不好的事情！请联系Lucky_fish0w0进行错误排查" + SuffixGenerator.getSuffix());
        }
    ...
    @Handler
    public void onCommand(CommandSender sender, long qq, String reason) {
        try {
            if (sender instanceof MemberCommandSender) {
                MemberCommandSender mcs = (MemberCommandSender) sender;
                if (!WhiteList.isGroupWhitelisted(mcs.getGroup().getId())) {
    ...
    @Handler
    public void onCommand(CommandSender sender, At at, String reason) {
        onCommand(sender, at.getTarget(), reason);
    }
}
